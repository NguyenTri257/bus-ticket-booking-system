...
// Trip service routes - proxy to trip-service microservice
app.use('/trips', async (req, res) => {
  try {
    const tripServiceUrl = process.env.TRIP_SERVICE_URL || 'http://localhost:3005';
    const queryString = Object.keys(req.query).length ? '?' + new URLSearchParams(req.query).toString() : '';
    console.log(`🔄 Proxying ${req.method} ${req.originalUrl} to ${tripServiceUrl}${req.path}${queryString}`);
    const response = await axios({
      method: req.method,
    ...
    console.log(`✅ Trip service responded with status ${response.status}`);
    // Forward all headers from the response
    Object.keys(response.headers).forEach(key => {
      res.set(key, response.headers[key]);
    });
    ...
    if (error.response) {
      console.log(`❌ Trip service responded with error status ${error.response.status}`);
      res.status(error.response.status).json(error.response.data);
    } else {
      console.log(`❌ Trip service unavailable or timeout`);
      res.status(500).json({
        success: false,
        error: { code: 'GATEWAY_003', message: 'Trip service unavailable' },
        timestamp: new Date().toISOString()
      });
    }
  }
...
if (process.env.NODE_ENV !== 'test') {
  app.listen(PORT, () => {
    ...
    console.log(`🔍 Trip Service: ${process.env.TRIP_SERVICE_URL || 'http://localhost:3005'}`);
